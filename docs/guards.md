...
| **Overwriting smart chips, math equations, inline images, horizontal rules, or footnotes** | `Engine` | Refuses mutating or removing smart chips, math equations, inline images (except in-place `innerText`, which keeps images), horizontal rules, or footnote references unless `force: true` is passed. | The Google Docs REST API cannot construct or restore rich smart chips, LaTeX equations, Drive-hosted images, horizontal rule dividers, or footnote text; replacing or deleting the containing paragraph destroys them permanently. |
