...
11. **Trust the result; do not read back after every write:** A `run` is atomic — if it returns an error, nothing in that batch was applied, so there is no partial state to inspect. A mutation step that changes nothing is rejected rather than reported as success (see `allowNoop`), so `ok: true` means every step actually changed the document. Batch the whole edit and verify once at the end, if at all. Re-querying between mutations costs round-trips without revealing anything the result did not already tell you.
12. **`contains` is a substring match, not a word match:** It is case-insensitive with no word boundaries, so `contains: "ci"` also matches "efficient" and "decisions", and `contains: "D4"` matches "D40". Every match echoes a `match` snippet with the hit bracketed (`…is more effi[ci]ent…`) — check it to confirm you matched what you meant. Filtered queries have no node-count cap, so an over-broad filter is rejected once the serialized result grows too large to read; narrow with `tab`, `nodeUnder`, `nodeKinds`, or a longer term, or use `output: "markdown"` for a broad read.
